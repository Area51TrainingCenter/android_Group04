		...
		try {
			...
			if( jsonData.getString( Constantes.CONSTANTE_RESPUESTA )
				...
				for (int i = 0; i < total ; i++) {
					...
					//Guardamos variables en el objeto usuario
					Constantes.objUsuario = 
							new Usuario( objectDato.getInt( Constantes.VAR_USUARIO_ID ) , 
									...
									objectDato.getString( Constantes.VAR_USUARIO_ESTADO ));
					
					//Por si acaso insertar en la base de datos  :D 
					ManageTableUsuario objTableUsuario = new ManageTableUsuario(this);
					objTableUsuario.ingresarUsuario();
